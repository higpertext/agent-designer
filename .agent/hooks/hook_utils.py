    ...
    for env_path in candidates:
        ...
        if env_path.exists():
            try:
                ...
                pass
    # Fallback: CWD si contiene pyproject.toml (indica que somos la raíz del proyecto)
    cwd = Path.cwd()
...
def get_htx() -> str:
    ...
    (única fuente de verdad, lee htx_config.json)."""
    root = get_project_root()
    try:
        from higpertext.kernel.htx_resolver import get_htx_cmd

        return get_htx_cmd(root)[0]
