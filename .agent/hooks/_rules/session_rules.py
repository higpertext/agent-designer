...
def _get_htx(root: Path) -> list[str]:
    """Delega en htx_resolver.get_htx_cmd — única fuente de verdad (lee htx_config.json)."""
    if get_htx_cmd is None:
        return ["htx"]
    try:
        return get_htx_cmd(root)
    except Exception as exc:
        _log.warning(f"[session] No se pudo resolver htx: {exc}")
        return ["htx"]
...
def inject_session_status(root: Path) -> RuleResult:
    ...
    if workflows:
        ...
        for wf in workflows[:6]:
            wf_lines.append(f"│  {wf['id']} → {wf.get('command', '—')}")
        if len(workflows) > 6:
            wf_lines.append(f"│  … {len(workflows) - 6} workflow(s) más")
        wf_lines.append("╚───────────────────────────────────────────────────────")
        lines.append("\n".join(wf_lines))
...
def _available_workflows(root: Path, profile: str) -> list[dict]:
    profile_data = _read_json(root / "src" / "config" / "profiles" / f"{profile}.json")
    declared = profile_data.get("workflows", [])
    if declared:
        return declared

    profile_lower = profile.lower()
    workflows_dir = root / "src" / "config" / "workflows"
    ...
    for path in sorted(workflows_dir.glob("*.json")):
        ...
        required = str(data.get("required_profile", "")).lower()
        if required not in {"global", profile_lower}:
            continue
        short_id = str(data.get("id", path.stem)).split(".")[-1]
        workflows.append(
            {
                "id": data.get("id", short_id),
                "when": data.get("description", data.get("name", "—")),
                "command": f"htx workflow run {short_id}",
            }
        )
    return workflows
